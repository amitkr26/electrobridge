...
import type { ResumeData, ProjItem } from "../../types";
...
function emptyProj(): ProjItem {
  return { id: crypto.randomUUID(), name: "", technologies: "", link: "", detail: "" };
}

export default function ProjectsSection({ data, onChange, onRequestAI, isAILoading }: ProjectsSectionProps) {
  const [expandedIndex, setExpandedIndex] = useState<number | null>(null);

  const updateItem = (index: number, field: keyof ProjItem, value: string) => {
  ...
  const addItem = () => {
    const updated = [...data.projects, emptyProj()];
    onChange({ ...data, projects: updated });
            ...
            <div key={item.id ?? index} className="border border-slate-200 rounded-xl overflow-hidden">
              ...
              {isExpanded && (
                <div className="p-4 space-y-3 border-t border-slate-100">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="Neural Network Accelerator"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        placeholder="Verilog, AXI, Python"
                      />
                    </div>
                  </div>
                  <div>
                  ...
                  >
                    <Sparkles size={14} />
                    {isAILoading ? "Polishing..." : "AI Project Polish"}
                  </button>
                </div>
              )}
            </div>
