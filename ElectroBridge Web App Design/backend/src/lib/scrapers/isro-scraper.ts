...
export async function scrapeISRO(): Promise<ScrapedOpportunity[]> {
  ...
  try {
    ...
    if (!res.ok) { console.error(`ISRO scraper: HTTP ${res.status}`); return []; }
    ...
    $("tr").each((_, row) => {
      ...
      const fullUrl = href
        ? href.startsWith("http") ? href : `https://www.isro.gov.in${href.startsWith("/") ? "" : "/"}${href}`
        : ISRO_URL;
      opportunities.push({
        title, organization: "ISRO", category: inferCategory(title),
        location: inferLocation(text), stipend: null, deadline: extractDeadline(text),
        eligibility: null, description: text.substring(0, 300),
        apply_link: fullUrl, source_url: ISRO_URL, tags: inferTags(title, text),
      });
    });
  } catch (error) { console.error("Error scraping ISRO:", error); }
