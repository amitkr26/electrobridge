...
function inferDeadline(description: string): string | null {
  ...
  for (const p of patterns) { const m = description.match(p); if (m) return m[1]; }
  return null;
}

function inferLocation(title: string, description: string): string | null {
  const combined = `${title} ${description}`;
  const cities = ["Bengaluru", "Bangalore", "Delhi", "Hyderabad", "Chennai", "Kolkata", "Mumbai", "Pune", "Chandigarh", "Lucknow", "India", "Singapore", "Germany", "UK", "USA", "Europe", "Netherlands", "France", "Switzerland", "Japan"];
  for (const city of cities) { if (combined.includes(city)) return city; }
...
async function scrapeCSIR_RSS(): Promise<ScrapedOpportunity[]> {
  ...
  try {
    const parser = new Parser({ timeout: 8000, headers: { "User-Agent": "Mozilla/5.0 (ElectroBridge/1.0)" } });
    const feed = await parser.parseURL("https://www.csir.res.in/en/rss.xml");
    for (const item of feed.items) {
      const title = item.title?.trim();
      if (!title || title.length < 10) continue;
      const skipPatterns = [/what is the/i, /can the following/i, /are there/i, /do committee/i, /are separate/i, /name and other/i, /assessment/i, /selection commi/i, /criteria/i, /basis of awarding/i, /mode of assessment/i, /rab|research assessment board/i];
      if (skipPatterns.some((p) => p.test(title))) continue;
      const link = item.link?.trim() || "";
      results.push({
        title, organization: "CSIR", category: inferCategory(title),
        location: "India", stipend: null, deadline: null, eligibility: null,
  ...
  } catch (error) { console.error("CSIR RSS scraper failed:", error); }
  return results;
}

export async function scrapeGovtJobs(): Promise<{ opportunities: ScrapedOpportunity[]; results: ScrapeResult[]; total: number }> {
