...
const RESULT_PATTERNS = [
  /list of selected/i, /list of provisionally/i, /provisional selected/i,
  /result for selection/i, /second list/i, /corrigendum/i, /answer key/i,
  /revised.*list/i, /validity of the selection/i, /extended upto/i, /revised final/i,
];
...
function inferLocation(text: string): string | null {
  const cities = ["Bengaluru", "Bangalore", "Delhi", "New Delhi", "Hyderabad", "Pune", "Chennai", "Kolkata", "Mumbai", "Chandigarh", "Gwalior", "Kanpur", "Tezpur", "Nasik", "Ambernath", "Visakhapatnam", "Dehradun", "Haldwani"];
  for (const city of cities) { if (text.includes(city)) return city; }
  return "India";
}
...
export async function scrapeDRDO(): Promise<ScrapedOpportunity[]> {
  ...
  try {
    ...
    if (!res.ok) { console.error(`DRDO scraper: HTTP ${res.status}`); return []; }
    ...
    titles.each((i, el) => {
      ...
      opportunities.push({
        title, organization: "DRDO", category: inferCategory(title),
        location: inferLocation(title + " " + descText), stipend: null,
        deadline: extractDeadline(title + " " + descText), eligibility: null,
        description: descText.substring(0, 300), apply_link: DRDO_VACANCIES_URL,
        source_url: DRDO_VACANCIES_URL, tags: inferTags(title, descText),
      });
    });
  } catch (error) { console.error("Error scraping DRDO:", error); }
