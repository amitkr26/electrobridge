...
export async function generateWeeklyDigest(opportunities: any[], newsArticles: any[]): Promise<string> {
...
Return plain text (not HTML, not JSON).`;
  ...
  const response = await callAI(prompt, undefined, { feature: "newsletter" });
